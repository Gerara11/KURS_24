...
def BOJ(B1: Fighter,B2: Fighter):
    ...
    while c==0 :
        ...
        if B1.get_as() >= B2.get_as() :
            t=t+B1.get_as()
            B2.set_hp(B2 - B1)
            Write(B1.get_name(), hp2-B2.get_hp(), B1.get_hp(), B2.get_hp(), t)
            if (B2.get_hp() == 0):
                c = 1
                with open("logger.txt", 'a') as f:
                    f.write(B1.get_name())
            ...
            t = t + B2.get_as()
            B1.set_hp(B1 - B2)
            Write(B2.get_name(), hp1 - B1.get_hp(),B1.get_hp(), B2.get_hp(), t)
            if (B1.get_hp() == 0):
                c = 1
                with open("logger.txt", 'a') as f:
                    f.write(B2.get_name())
...
H = Fighter("Horus", 100,[0,25],[1,10])
S = Fighter("Sanguinius", 100,[0,15],[5,10])
BOJ(H,S)
